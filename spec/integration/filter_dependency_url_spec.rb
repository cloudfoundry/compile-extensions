...
describe 'filter_dependency_url' do
  def run_filter
  ...
  context 'with credentials in the url' do
    ...
    context 'the uri is a file generated by buildpack packaging' do
      let(:credentials)  { 'log_in_p!%40assword' }
      let(:redacted)     { '-redacted-_-redacted-' }
      let(:original_url) { "file:///tmp/buildpacks/f884f12a2292a8919ef4faf07d212635/dependencies/https___#{credentials}@buildpacks.cloudfoundry.org_concourse-binaries_nginx_nginx-1.11.3" }

      it 'redacts the credentials' do
        filtered_url, _, _ = run_filter

        expect(filtered_url).to eq("file:///tmp/buildpacks/f884f12a2292a8919ef4faf07d212635/dependencies/https___#{redacted}@buildpacks.cloudfoundry.org_concourse-binaries_nginx_nginx-1.11.3\n")
      end
    end
  ...
  context 'with no credentials in the url' do
    ...
    context 'the uri is a file generated by buildpack packaging' do
      let(:original_url) { "file:///tmp/buildpacks/f884f12a2292a8919ef4faf07d212635/dependencies/https___buildpacks.cloudfoundry.org_concourse-binaries_nginx_nginx-1.11.3" }

      it 'redacts the credentials' do
        filtered_url, _, _ = run_filter

        expect(filtered_url).to eq("file:///tmp/buildpacks/f884f12a2292a8919ef4faf07d212635/dependencies/https___buildpacks.cloudfoundry.org_concourse-binaries_nginx_nginx-1.11.3\n")
      end
    end
